...
class KotlinCodeBlockUtils {
    ...
    companion object {
        ...
        fun insertCodeBlock(project: Project, templateName: String, activityInjector: PsiFile, properties: Properties) {
            ...
            val ktBlockCodeFragment =
                KtBlockCodeFragment(project, templateName, internalTemplate.getText(properties), null, null)
            PsiDocumentManager.getInstance(project)
                .commitDocument(PsiDocumentManager.getInstance(project).getDocument(activityInjector)!!)

            val e = object : PsiRecursiveElementVisitor() {
                override fun visitElement(element: PsiElement?) {
                    if (element is KtClassBody) {
                        element.addRangeBefore(
                            ktBlockCodeFragment.firstChild,
                            ktBlockCodeFragment.lastChild,
                            element.rBrace
                        )
                        PsiDocumentManager.getInstance(project)
                            .commitDocument(PsiDocumentManager.getInstance(project).getDocument(activityInjector)!!)
